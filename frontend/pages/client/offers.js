...
import { ApplyApi, GetApplicationByUserApi, getOffersApi } from "../api/others";
...
const OffersPage = () => {
    ...
    return (
        <div className="min-h-screen flex flex-col">
            ...
            {/* ================= Main ==================== */}
            <main>
            <h2 className="m-5 text-gray-800 text-3xl font-bold">Offers List</h2>
                <div className="flex flex-wrap items-center justify-center bg-gray-100">
                        {offers?.map((offer) => (
                            <div key={offer.id} className="max-w-sm rounded overflow-hidden shadow-lg m-5">
                                ...
                                <div className="px-6 pt-4 pb-2">
                                    <button onClick={() => router.push('/client/offer-details?offerId='+offer.id)} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                                        Details
                                    </button>
                                </div>
                            </div>
